...
async function startPlaying(message) {
    ...
    try {
        const maxSeekSeconds = (song.duration_ms / 1000) - 30;
        const minSeekSeconds = 5;

        const randomSeek = Math.floor(Math.random() * (maxSeekSeconds - minSeekSeconds + 1)) + minSeekSeconds; // random between any two numbers
        const stream = await playdl.stream(link, { filter: 'audioonly', seek: randomSeek, highWaterMark: 1 << 23, type: 'opus', dlChunkSize: 0 });
